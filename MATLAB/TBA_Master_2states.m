...
t = linspace(0,2*pi);
plot(C8(:,1)+ R*cos(t), C8(:,2)+ R*sin(t),'g-')

axis square
axis xy

r1 = hypot(PCA_data(:,1)-C8(:,1), PCA_data(:,2)-C8(:,2));

plot(PCA_data(r1<=R,1),PCA_data(r1<=R,2),'go')

s8 = [PCA_data(r1<=R,1), PCA_data(r1 <=R,2)];

hold on 

plot(C9(:,1)+ R*cos(t), C9(:,2)+ R*sin(t),'r-')
r2 = hypot(PCA_data(:,1)-C9(:,1), PCA_data(:,2)-C9(:,2));

plot(PCA_data(r2<=R,1),PCA_data(r2<=R,2),'ro')

s9 = [PCA_data(r2<=R,1), PCA_data(r2<=R,2)];
...
dlmwrite('dcsa_310_rvals.txt', r_values, 'delimiter','\t') ; %write out to txt file
...
dlmwrite('dcsa_310_state_tba.txt', State_TBA, 'delimiter','\t') ; %write out to txt file
...
dlmwrite('dcsa_310K_S1_Frames_12.txt', f1, 'delimiter','\t') ; 
dlmwrite('dcsa_310K_S2_Frames_12.txt', f2, 'delimiter','\t') ; 
%dlmwrite('dcsa_310K_S3_Frames_12.txt', f3, 'delimiter','\t') ; 
%dlmwrite('dcsa_310K_S4_Frames_13.txt', f4, 'delimiter','\t') ; %write out states to txt file
...
dlmwrite('dcsa_310_TBA_check.txt', comp, 'delimiter','\t') ; %write out states to txt file
dlmwrite('dcsa_310_TBA_states.txt', State', 'delimiter', '\t');
...
VLM_state=[val1', len1']; 
dlmwrite('dcsa_310_Len_Vals_check.txt', VLM_state, 'delimiter', '\t'); %write out to txt file
...
%here sum(C(i,l))==N(i)
dlmwrite('csa_310_count_matrix.txt', CM_test, 'delimiter', '\t'); 
dlmwrite('csa_310_count_matrix_TBA.txt', CM, 'delimiter', '\t'); 
...
dlmwrite('dcsa_310_prob_matrix.txt', PM, 'delimiter', '\t'); %write out to txt file
...
dlmwrite('310_dcsa_rate_matrix.txt', K, 'delimiter', '\t'); %write out to txt file
...
st = [t , states];
dlmwrite('dcsa_310K_TBA_traj.txt', st, 'delimiter','\t') ;
