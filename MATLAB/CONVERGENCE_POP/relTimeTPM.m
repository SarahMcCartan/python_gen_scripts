%% FUNCTION TO FIND RELAXATION TIME FROM A TRANSITION PROB MATRIX
...
function f = relTimeTPM(in,out,data,lag,ns,temp,pep)
...
for k = 1 : len
    ...
    rel_time = -1*(lagdata(k)./log(abs(D))); % work out relaxation time in frames
    ...
    fout3 = fullfile(OutFolder, sprintf('%d%s_PeqEig%06d.txt',temp,pep,k));
    fout4 = fullfile(OutFolder, sprintf('%d%s_RELT%06d.txt',temp,pep,k));
    
    dlmwrite(fout1, D, 'delimiter', '\t');
    dlmwrite(fout2, V, 'delimiter', '\t');
    dlmwrite(fout3, peq, 'delimiter','\t');
    dlmwrite(fout4, rel_time, 'delimiter', '\t');
    
end
%write out slowest relaxation time vs lag time in frames
